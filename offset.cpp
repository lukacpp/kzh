...
DWORD cOffset::FindClientTable()
{
	...
	if (PatternAddress)
	{
		for (byte i = 0; i < sizeof(ClientOffset); i++)
		{
			...
			if (!FarProc((DWORD)ClientTablePtr, HwBase, HwEnd) && !IsBadReadPtr((PVOID)ClientTablePtr, sizeof(cl_clientfunc_t)))
...
DWORD cOffset::FindStudioTable()
{
	...
	if (FarProc((DWORD)StudioTablePtr, HwBase, HwEnd) && FarProc((DWORD)StudioTablePtr, HlBase, HlEnd) && FarProc((DWORD)StudioTablePtr, ClBase, ClEnd))
	{
...
{
	BYTE Pattern_UserMsg[9] = { 0x52, 0x50, 0xE8, 0xFF, 0xFF, 0xFF, 0xFF, 0x83, 0x00 };
	BYTE Pattern_UserMsg2[13] = { 0xFF, 0xFF, 0xFF, 0x0C, 0x56, 0x8B, 0x35, 0xFF, 0xFF, 0xFF, 0xFF, 0x57, 0x00 };
